...
class BranchingDQN(nn.Module):
    ...
    def __init__(self, obs, ac, n, config):
        ...
        for name,param in self.q.named_parameters():
            i=i+1
            print("Parameter Nr:",i,"Name Parameter", name)
            #print(param)

        self.target.load_state_dict(self.q.state_dict())

        print("Initalisierung BranchingDQN fertig - zwei Netze q und target erstellt. Target Netz hat gleiche Parameter wie online(q) Netz")
    ...
    def update_policy(self, adam, memory, params):
        ...
        with torch.no_grad():
            ...
            # max_next_q_vals = max_next_q_vals.mean(1, keepdim=True).expand(-1,4) #Version, die ohne Warnung lauft
        ...
        if self.update_counter % self.target_net_update_freq == 0:
            ...
            # Ursprüngliche Update Funktion: Gewichte+Bias werden gesamthaft geupdated
            if config.tau == 0:
                self.target.load_state_dict(self.q.state_dict())

            # Mixing von Gewichten und Bias mit Tau

            else:
                print("Mixing with Tau= ", config.tau)
                for param_target, param_online in zip(self.target.parameters(), self.q.parameters()):
                    #print("\nTarget Parameter vor Mixing:", param_target)
                    param_target.data = (1.0 - config.tau)*param_target+config.tau*param_online
                    #print("\nTarget Parameter Nach Mixing:", param_target)
            print("\nMxing Done")
